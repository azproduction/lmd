    ...
    return this.resolveGlobedFiles(modulesRoot)
        .then(function (allFiles) {
            ...
            var promises = self.files.map(function (file) {
                return file.withLogger(self.logger).load(modulesRoot);
            });
 ...
 * @return {String}
 */
CommonModule.prototype.getDependsConfigOf = function (fileName, dependsMask) {
 ...
 * @returns {Array}
 */
CommonModule.prototype.resolveModulesDepends = function (defaultDependsMask) {
    var self = this;
...
CommonModule.prototype._resolvePluginsAndDepends = function () {
    ...
    if (this.type === 'amd') {
        this.addRequiredPlugin('amd');
    }

    var moduleAnalytics = moduleAnalyzer(this);

    moduleAnalytics.requiredPlugins.forEach(function (pluginName) {
        self.addRequiredPlugin(pluginName);
    });

    moduleAnalytics.requiredModules.forEach(function (moduleName) {
        self.addRequiredModule(moduleName);
    });
};
