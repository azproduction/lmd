...
var BUNDLE_INHERIT_FROM_PACKAGE = ['root', 'warn', 'log', 'depends', 'lazy', 'pack', 'pack_options', 'main'],
...
function flattenBundles(config, namespace) {
    ...
    Object.keys(config.bundles).forEach(function (bundleName) {
        ...
        // Flatten sub-bundles
        if (bundle.bundles) {
            var subBundles = flattenBundles(bundle, bundleName + SUB_BUNDLE_SEPARATOR);
...
var bundleInheritFieldsFromPackage = function (bundleName, bundle, config) {
    ...
    if (!bundle.styles_output && config.styles_output) {
        // using parent output, set output path for bundle
        // ../index.css -> ../index-bundle-%bundleName%.css
        bundle.styles_output = config.styles_output.replace(/\.css$/, '') + '-bundle-' + bundleName + '.css'
    }
