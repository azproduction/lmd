...
var BUNDLE_INHERIT_FROM_PACKAGE = ['root', 'warn', 'log', 'depends', 'lazy', 'pack', 'pack_options'];
...
var mergeConfigs = function (configA, configB, flagsNames, inheritableFields, isMasterConfig, context) {
    ...
    for (var bundleName in configB.bundles) {
        // Warn if bundle exists an its not a master config
        if (!isMasterConfig && configA.bundles[bundleName]) {
            configA.errors.push('Name conflict! Bundle **"' + bundleName + '"** will be overwritten by ' + context);
        }
...
var assembleLmdConfig = function (configFile, flagsNames, extraOptions, usedConfigs) {
    ...
    addBundlesCallback(rawConfig);
    var config = assembleLmdConfigAsObject(rawConfig, configFile, configDir, flagsNames, extraOptions, usedConfigs);
...
/**
 * Creates random bundle callback if not defined
 *
 * @param resultConfig
 */
function addBundlesCallback(resultConfig) {
    if (resultConfig.bundles && !resultConfig.bundles_callback) {
        resultConfig.bundles_callback = '_' + Math.round(Math.random() * 0xFFFFFFFF).toString(16);
    }
}
...
function addPluginsFromBundles(resultConfig) {
    if (resultConfig.bundles) {
        ...
        Object.keys(resultConfig.bundles).forEach(function (bundleName) {
            mergeFlags(resultConfig, resultConfig.bundles[bundleName], Object.keys(LMD_PLUGINS), false);
        });
    }
}
...
var bundleInheritFieldsFromPackage = function (bundleName, bundle, config) {
    ...
    // Set output path
    if (!bundle.output && config.output) {
        // using parent output, set output path for bundle
        // ../index.js -> ../index-bundle-%bundleName%.js
        bundle.output = config.output.replace(/\.js$/, '') + '-bundle-' + bundleName + '.js'
    }
    ...
    // Override
    bundle.bundles_callback = config.bundles_callback;
};
