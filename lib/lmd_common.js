...
var isRequireCall = function (ast, requireName) {
    if (ast[0] !== "call") {
        return false;
    }

    while (true) {
        if (ast[1][0] === "name" && ast[1][1] === requireName) {
            return true;
        } else if (ast[1][0] === "dot") {
            ast = ast[1];
        } else {
...
var findRequireCalls = function (ast, moduleType) {
    var requireName = ast[1] && ast[1][0] && ast[1][0][2] && ast[1][0][2][0],
        extraRequireName,
        requireCalls = [];
    ...
    walker.with_walkers({
        "call": function () {
            if (isRequireCall(this, requireName)) {
                requireCalls.push(this);
            }
        }
    }, function () {
...
var findModuleRequireFeatures = function (requireExpressions) {

};

var findModuleRequirements = function (requireExpressions, moduleType) {
...
var analiseModuleContent = function (moduleOptions) {
    ...
    var requireExpressions = findRequireCalls(ast, moduleDescriptor.type);
    // console.log(requireExpressions);

    moduleDescriptor.features = findModuleRequireFeatures(requireExpressions);
    moduleDescriptor.requires = findModuleRequirements(requireExpressions, moduleDescriptor.type);
