...
var MASTER_FIELDS = INHERITABLE_FIELDS.concat(['output', 'path', 'root', "sourcemap", "sourcemap_inline", "sourcemap_www", "www_root"]);
        ...
        if (IS_HAS_WILD_CARD.test(modulePath)) {
            ...
            fs.readdirSync(path.join(modulesDirPath, moduleFilePath)).forEach(function (fileName) {
                ...
                if (match) {
                    ...
                    moduleRealPath = path.join(modulesDirPath, moduleFilePath, fileName);
                    moduleExists = true;
