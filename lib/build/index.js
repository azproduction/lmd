...
Build.prototype.load = function () {
    ...
    return this.promise = this.readConfig(this.configFile)
        .then(function (config) {
            self.logger.log('Config loaded <id>' + self.id + '</id> file <file>' + self.configFile + '</file>');
            self.isExists = true;
            self.config = config;

            return self.configure();
        }, function (error) {
            self.logger.error('Failed to load config <id>' + self.id + '</id> file <file>' + self.configFile + '</file>');
...
Build.prototype.configure = function () {
    ...
    return this.extendsBy(self.config['extends'])
        ...
        .then(function () {
            self.logger.log('Loading main bundle of <id>' + self.id + '</id> file <file>' + self.configFile + '</file>');
...
Build.prototype.extendsBy = function (configFile) {
    ...
    self.logger.log('Extending <id>' + self.id + '</id> file <file>' + self.configFile + '</file> with <file>' + configFile + '</file>');
    return build.withLogger(this.logger).load().then(function () {
        self.logger.log('Merging <id>' + self.id + '</id> file <file>' + self.configFile + '</file> with <file>' + configFile + '</file>');
...
Build.prototype.addModulesDepends = function () {
    ...
    self.logger.log('Applying modules depends of <id>' + self.id + '</id> file <file>' + self.configFile + '</file>');
...
Build.prototype.mixWith = function (buildNames) {
    ...
    this.logger.log('Mixing <id>' + this.id + '</id> file <file>' + this.configFile + '</file> with ' + buildNames.join(', '));
    return Q.resolve();
};
