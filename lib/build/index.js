...
Build.prototype.configure = function () {
    ...
    return this.extendsBy(self.config['extends'])
        ...
        .then(function () {
            ...
            return self.bundle.withLogger(self.logger).load(self.modulesRoot);
        }, function (error) {
        ...
        .then(function () {
            self.logger.log1('Loading lmd source', self.id, self.configFile);
            self.lmdSource = new LmdSource(self.config);
            return self.lmdSource.withLogger(self.logger).load();
        });
};
...
Build.prototype.mixWith = function (buildNames) {
    ...
    var promises = buildNames.map(function (configFile) {
        ...
        return build.withLogger(self.logger).load();
    });

    return Q.all(promises).spread(function () {
        Array.slice.call(arguments).forEach(function (build) {
            self.logger.log1('Merging %id file %file with %file', self.id, self.configFile, build.configFile);
            self.mergeConfigWith(build, INHERITABLE_FIELDS);
        });
        return self;
    });
