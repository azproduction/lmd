...
Bundle.prototype.load = function (modulesRoot) {
    ...
    var promises = this.bundles.map(function (bundle) {
        return bundle.withLogger(self.logger).load(modulesRoot);
    });
    // load modules collection
    promises.push(this.modulesCollection.withLogger(self.logger).load(modulesRoot));
 ...
 * @returns {Bundle}
 */
Bundle.prototype.withLogger = function (logger) {
    this.logger = logger;
