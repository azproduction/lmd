...
LmdWriter.prototype._pipeStreamToFile = function (stream, fileName, onAllEnd) {
    if (this._isCanWriteStream(stream, fileName)) {
        ...
        // listen on 'close' to ensure that the file
        // has been properly flushed
        // http://stackoverflow.com/a/14641886
        writer.on('close', onAllEnd);
        stream.pipe(writer);
    } else {
        onAllEnd();
    }
...
LmdWriter.prototype._writeStreamsTo = function (stream, paths, resourceName, onAllEnd) {
    this._logResult(stream, paths.source, 'LMD ' + resourceName);
    this._pipeStreamToFile(stream, paths.source, onAllEnd);

    this._logResult(stream.sourceMap, paths.sourceMap, 'Source Map of ' + resourceName);
    this._pipeStreamToFile(stream.sourceMap, paths.sourceMap, onAllEnd);
};
...
LmdWriter.prototype.writeAll = function (callback) {
    ...
    var onAllEnd = this._onAllEnd(callback);
    ...
    // Print package
    this._writeStreamsTo(source, paths, 'Package', onAllEnd);
    ...
    Object.keys(bundles).forEach(function (bundleName) {
        ...
        self._writeStreamsTo(stream, paths, 'Bundle ' + bundleName.green, onAllEnd);
    });
